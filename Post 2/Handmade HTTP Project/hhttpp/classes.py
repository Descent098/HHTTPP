...
class Server:
    ...
    def parse_request(self, input_text:str) -> Request:
        ...
        # Parse headers TODO: TURN INTO FUNCTION
        regex = r"^(.*):(.*)$"
        headers_match = re.match(regex, input_text, re.MULTILINE)
        print(headers_match)
        if headers_match:
            print(f"{headers_match.groups()}")
            headers = dict()
        else:
            headers = dict()
        
        
        # Find content TODO: TURN INTO FUNCTION
        regex = r".*\n\n(.+)"
        content_match = re.match(regex, input_text, re.MULTILINE | re.DOTALL)
        if not content_match:
            content = ""
        else:
            print(f"Found content {content_match.group(1)}")
...
if __name__ == "__main__": # Code inside this statement will only run if the file is explicitly called and not just imported.
    ...
    test = """GET /search?q=test HTTP/1.1
